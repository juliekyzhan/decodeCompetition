...
# 特殊补丁：修改 Sen. Blunt Rochester, Lisa [D-DE] 的 chamber 和 district
legis_sum <- legis_sum %>%
  mutate(
    chamber = if_else(full_name == "Sen. Blunt Rochester, Lisa [D-DE]", 
                      "House of Representatives", chamber),
    district = if_else(full_name == "Sen. Blunt Rochester, Lisa [D-DE]", 
                       "At-Large", district)
  )

# 检查修改结果
legis_sum %>% 
  filter(full_name == "Sen. Blunt Rochester, Lisa [D-DE]") %>%
  select(full_name, chamber, district)
...
```{r name-missing-cleansing}

# 存在议员full_name缺失但是选区和年龄有

bioguide_path <- "/Users/kunyazhan/Desktop/data/new ideology/bioguide.xlsx"
bioguide <- read_xlsx(bioguide_path)


# ---- 对两个数据库都取eng name用于后续匹配正确的议员名称 ----


# 正则化表达提取州和选区
bioguide_name_map <- bioguide_name_map %>%
  mutate(
    # 提取 state
    state_code = str_extract(full_name, "(?<=\\[R-|\\[D-|\\[I-)[A-Z]{2}"),

    # 提取 district（含At Large）
    district = str_extract(full_name, "(?<=\\[[DRI]-[A-Z]{2}-)([0-9]+|At Large)(?=\\])"),

    # 参议员设定为99
    district = if_else(str_detect(full_name, "^Sen\\."), "99", district),

    # 提取名字部分并整理
    last_first = str_extract(full_name, "(?<=\\.).*?(?=\\[)") %>% 
      str_trim(),

    # 转换为 “名 姓” 格式
    eng_name = str_replace(last_first, "^([^,]+),\\s*(.+)$", "\\2 \\1")
  )

bioguide_name_map$district <- as.character(bioguide_name_map$district)

# 检查 bioguide_name_map 中 eng_name 是否唯一
dup_names <- bioguide_name_map %>%
  group_by(eng_name) %>%
  summarise(n = n_distinct(bioguide_id), .groups = "drop") %>%
  filter(n > 1)

if (nrow(dup_names) == 0) {
  cat("✅ 所有 eng_name 都唯一，可以安全按名字匹配。\n")
} else {
  cat("⚠️ 以下 eng_name 对应多个 bioguide_id，请注意存在同名议员：\n")
  print(dup_names)
}
...
special_clean_menendez <- function(df) {
  df <- df %>%
    mutate(
      eng_name = case_when(
        # 如果是参议员 Robert Menendez（州 NJ，district=99 或 0）
        full_name == "Sen. Menendez, Robert [D-NJ]" |
        (state_code == "NJ" & district %in% c(99, 0) & grepl("Menendez", full_name)) ~ "Bob Menendez",
        
        # 如果是众议员 Robert Menendez（NJ-8）
        full_name == "Rep. Menendez, Robert [D-NJ-8]" |
        (state_code == "NJ" & district == 8 & grepl("Menendez", full_name)) ~ "Rob Menendez",
...
bioguide_name_map <- special_clean_menendez(bioguide_name_map)
bioguide_name_map$congress <- as.character(bioguide_name_map$congress)


```


```{r adver-name-special clean}
...
adver_sum0 <- adver_sum %>%
  ...
  select(-name_no_title, -name_clean)  
...
fill_missing_full_name <- function(main_df, bioguide_map, verbose = TRUE) {
  # 找出 full_name 为空或 NA 的行
  missing_idx <- which(is.na(main_df$full_name) | main_df$full_name == "")
  
  if (length(missing_idx) == 0) {
    if (verbose) cat("✅ 没有缺失的 full_name，无需填充。\n")
    return(main_df)
  }
  
  # 从 bioguide_map 中根据 congress + state_code + district 匹配 full_name
  missing_df <- main_df[missing_idx, ]
  
  filled_df <- missing_df %>%
    left_join(
      bioguide_map %>% select(full_name, congress, state_code = state, district),
      by = c("congress", "state_code", "district")
    )
  
  # 用匹配到的 full_name 填充 main_df
  main_df$full_name[missing_idx] <- coalesce(filled_df$full_name, main_df$full_name[missing_idx])
  
  if (verbose) {
    cat("✅ 已填充缺失 full_name 的数量：", sum(!is.na(main_df$full_name[missing_idx])), "\n")
  }
...
# 使用示例
adver_sum0_clean <- fill_missing_full_name(adver_sum0, bioguide_name_map)


```

```{r legis-name-special clean}
...
legis_sum0 <- legis_sum0 %>%
  mutate(
    full_name = if_else(
      str_detect(full_name, "\\[.*\\]"),  # 检查是否包含中括号
      full_name,                          # 含中括号的保留
      NA_character_                        # 不含中括号的置为 NA
    )
  )

sum(is.na(legis_sum0$full_name))
...
# 定义函数：根据议员姓名和选区信息匹配 bioguide_id

match_bioguide_id <- function(main_df, bioguide_name_map, verbose = TRUE) {
  
  report_progress <- function(step, matched_df, total_n) {
    matched_n <- sum(!is.na(matched_df$bioguide_id))
    if (verbose) {
      cat(sprintf("Step %s: matched %d / %d (%.1f%%)\n",
                  step, matched_n, total_n, 100 * matched_n / total_n))
    }
  }
  
  total_n <- nrow(main_df)
  
  # Step 1: 第一轮匹配 —— state_code + district + eng_name
  matched_df <- main_df %>%
    left_join(
      bioguide_name_map %>% select(bioguide_id, state_code, district, eng_name),
      by = c("state_code", "district", "eng_name")
    )
  
  report_progress("1 (state+district+eng_name)", matched_df, total_n)
  
  # Step 2: 第二轮匹配 —— state_code + eng_name
  matched_df <- matched_df %>%
    left_join(
      bioguide_name_map %>% select(bioguide_id2 = bioguide_id, state_code, eng_name),
      by = c("state_code", "eng_name")
    ) %>%
    mutate(bioguide_id = coalesce(bioguide_id, bioguide_id2)) %>%
    select(-bioguide_id2)
  
  report_progress("2 (state+eng_name)", matched_df, total_n)
  
  # Step 3: 第三轮匹配 —— eng_name only
  matched_df <- matched_df %>%
    left_join(
      bioguide_name_map %>% select(bioguide_id3 = bioguide_id, eng_name),
      by = "eng_name"
    ) %>%
    mutate(bioguide_id = coalesce(bioguide_id, bioguide_id3)) %>%
    select(-bioguide_id3)
  
  report_progress("3 (eng_name only)", matched_df, total_n)
  
  if (verbose) {
    cat("✅ 匹配流程完成。\n")
    cat("最终匹配结果：\n")
    print(table(is.na(matched_df$bioguide_id)))
  }
...
adver_sum1 <- match_bioguide_id(adver_sum0, bioguide_name_map)
legis_sum1 <- match_bioguide_id(legis_sum0, bioguide_name_map)
...
generate_decision_with_traits <- function(df, mode = "all") {
  ...
  if (!mode %in% c("all", "train", "test", "merged")) {
    stop("mode 必须是 'all', 'train', 'test', 或 'merged'")
  }

  # ---- 1️⃣ 数据筛选 ----
  df_subset <- if (mode %in% c("all", "merged")) {
    df
  } else {
...
# adv
adver_cg <- generate_decision_with_traits(adver_sum, mode = "merged")

#df_train <- generate_decision_with_traits_dual(df, mode = "train")
#df_test <- generate_decision_with_traits_dual(df, mode = "test")

# legis
legis_cg <- generate_decision_with_traits(legis_sum, mode = "merged")
...
committee_summary_collapsed <- committee_decision_summary %>%
  mutate(
    decision_group = ifelse(decision == "dom", "intl", "intl_prof")
  ) %>%
  group_by(committee_el, decision_group) %>%
  summarise(group_percentage = sum(percentage), .groups = "drop") %>%
  pivot_wider(
...
issue_counts_mid <- bills_data_aug %>%
  group_by(congress, party, bioguide_id, issue) %>%
  summarise(n_issue_mentions = n(), .groups = "drop")
